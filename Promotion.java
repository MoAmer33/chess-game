...
public class Promotion extends JDialog implements ActionListener {
    ...
    Promotion(final JFrame frame,final Board board,final BoardGUI boardGUI)
    {
        super(frame, "Promotion",
            ModalityType.APPLICATION_MODAL); /* first argument means that this JDialog
                                            ...
                                            JDialg blocks the frame until it have been
                                            closed*/
    ...
    private void buildPromotionWindow(Position newPosition)
    {
        ...
        {  
            ...
            try
            {
                ...
                Image scaledImage = 
                        image.getScaledInstance(90, 90, Image.SCALE_SMOOTH);
                
                JLabel pic = new JLabel(new ImageIcon(scaledImage));
                pic.setSize(100,100);
                pic.setCursor(new Cursor(Cursor.HAND_CURSOR));
                pieces[i].add(pic); //Adding the piece piecture to the button
    ...
    public void promoting (Piece promotedPiece)
    {
        ...
        boardGUI.getSpot(Moving.getPromotionPosition()).
                addPieceImage(board, Moving.getPromotionPosition()); /* updating pawn
                                                                        to the new
                                                                        piece on gui*/
        
        boardGUI.displayLastMovePositions(); /* displaing this move positions 
                                                (Gold spots) */
        
        promoted = true; /* this means the palyer chose a piece and didn't close
                            the promotion window */
        
        
        this.dispose(); // Colsing the promotion Window
    }
    ...
    {
        if (newPosition == null || board.getSpot(newPosition).getHasPiece() == false)
        {
            return false;
        }
        else
        {
            return (isPawn(newPosition) &&
                    (newPosition.y == 1 || newPosition.y == 8));
        }
    }
    //////////////////////////////
