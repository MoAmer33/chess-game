...
public class Moving implements MouseListener, MouseMotionListener {
    ...
    public static Position Drag = null; /* position of the spot that piece drgged 
                                        from when you choose (Drag and Drop) method*/    
    
    public static Position Drop = null; /* position of the spot that piece dropped
                                            on when you choose(Drag and Drop) method*/
    
    private static Position firstSelectedSpot = null; /* position of the moving piece
                                                         when you choose 
                                                         click-to-move method*/
    
    private static Position secondSelectedSpot = null; /* position of the moving
                                                        piece final destination when
                                                        you choose click-to-move
                                                        method */
    
    private static JLabel c; // The moving piece
    
    private static int spotWidth; // Rhe spot width
    
    private static int spotHeight; // The spot height
    
    private static Position mouseSpotPos; // Position of the mouse inside a spot
    
    private static Position validPosition[]; //The valid moves of the moving piece
    
    private static int validLength; // "validPosition" array length
    ...
    private static Position lastPointedPosition; /* last spot postition that 
                                                    mouse pointed at */
 ...
 /*************************************************************************/
    ...
    public Moving(final JFrame frame ,final Board board, final BoardGUI boardGUI,
    ...
    public void secondTimeselecting(Point p)
    {
        ...
        if (secondSelectedSpot != null)
        {
            ...
            else
            {
                ...
                if (boardGUI.hasSamePlayerColor(firstSelectedSpot) &&
    ...
    public void mouseClicked(MouseEvent me)
    {
        if (me.getButton() == 1) // left button is clicked
        {
            ...
            // after second click when "secondSelectedSpot" has a value
            if (secondSelectedSpot != null)
            {
                firstSelectedSpot = null;
                secondSelectedSpot = null;
